...
// Paciente armazenado no nó AVL: id, nome, histórico opaco, flag chamado e prioridade.
...
static PatientNode *new_node(const char *id, const char *name) {
    ...
    strncpy(n->data.id, id, MAX_ID_LEN);
    n->data.id[MAX_ID_LEN] = '\0';

    strncpy(n->data.name, name, MAX_NAME_LEN);
    n->data.name[MAX_NAME_LEN] = '\0';
...
static PatientNode *avl_remove(PatientNode *root, const char *id, int *rcode) {
    ...
    if (cmp < 0)
        ...
        // encontrado: NÃO destruir history aqui...
        if (!root->left || !root->right) {
            ...
            if (root->data.hist) 
                history_destroy(root->data.hist);
...
/* Prioridade do paciente (1..5): setters/getters */
